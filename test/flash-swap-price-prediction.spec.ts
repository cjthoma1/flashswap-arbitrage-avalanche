...
import { isLocalEnv, expandTo18Decimals, bigNumberToNumber } from '../shared/utilities';
...
describe('FlashSwapPricePrediction', () => {
    ...
    beforeEach(async () => {
        if (isLocalEnv(network.name)) {
            const { WAVAX, USDT, PANGO_COMPUTE_LQUIDITY, PANGO_WAVAX_USDT_PAIR, UNISWAP_COMPUTE_LQUIDITY, UNISWAP_WAVAX_USDT_PAIR } = await setupTest();
            Wavax = WAVAX;
            Usdt = USDT;
    ...
    beforeEach('mint some liquidity for Pango and Uniswap/Sushiswap', async () => {
    ...
    describe('#getReservesAfterArbitrage', () => {
        it('should predict arbitrage swap for for 1 AVAX / 100 X on Pango and 1 AVAX / 200 X on Sushiswap', async () => {
            ...
            const [pangoReserveA, pangoReserveB] = await PangoComputeLiquidityValue.getReservesBeforeArbitrage(
                token0,
                token1,
                amount0,
                amount1
            ...
            const [uniswapReserveA, uniswapReserveB] = await UniswapComputeLiquidityValue.getReservesDuringArbitrage(
                token0,
                token1,
                amount0,
                amount1
