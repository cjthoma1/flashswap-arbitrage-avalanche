...
describe('SushiswapV2ComputeLiquidityValue', () => {
	let Wavax: Contract;
	let Usdt: Contract;
	...
	beforeEach(async () => {
		if (isLocalEnv(network.name)) {
			const { WAVAX, USDT, UNISWAP_COMPUTE_LQUIDITY, UNISWAP_WAVAX_USDT_PAIR } = await setupTest();
			  Wavax = WAVAX;
			  Usdt = USDT;
              ComputeLiquidityValue = UNISWAP_COMPUTE_LQUIDITY;
			  WavaxUsdtPair = UNISWAP_WAVAX_USDT_PAIR;
		}
	});
    ...
    describe('#getReservesAfterArbitrage', () => {
      ...
      describe('#getReservesBeforeArbitrage', () => {
        it('1/100', async () => {
            ...
            const [reserveA, reserveB] = await ComputeLiquidityValue.getReservesBeforeArbitrage(
                token0,
                token1,
                amount0,
                amount1
    ...
    describe('#getReservesDuringArbitrage', () => {
        it('1/100', async () => {
            ...
            const [reserveA, reserveB] = await ComputeLiquidityValue.getReservesDuringArbitrage(
                token0,
                token1,
                amount0,
                amount1
