...
import { expandTo18Decimals, bigNumberToNumber, isLocalEnv } from '../shared/utilities';
import { expect } from 'chai';
import setupTest from './test-fixture';

describe('FlashSwappySushi', () => {
	let FlashSwappySushi: Contract;
	let Wavax: Contract;
	let Usdt: Contract;
	...
	beforeEach(async () => {
		if (isLocalEnv(network.name)) {
			const { WAVAX, USDT, PANGO_WAVAX_USDT_PAIR, UNISWAP_WAVAX_USDT_PAIR, FLASH_SWAP_SUSHI } = await setupTest();
			Wavax = WAVAX;
			Usdt = USDT;
			PangoWavaxUsdt = PANGO_WAVAX_USDT_PAIR;
			SushiSwapWavaxUsdt = UNISWAP_WAVAX_USDT_PAIR;
			FlashSwappySushi = FLASH_SWAP_SUSHI;
		}
	});
	...
	it('should send user 69 USDT', async function () {
		...
		const amount0 = SushiPairToken0 === Usdt.address ? expandTo18Decimals(0) : arbitrageAmount;
		// If token0 is USDT then set amount1 to 1 else set its value to 0 since its USDT
		const amount1 = SushiPairToken0 === Usdt.address ? arbitrageAmount : expandTo18Decimals(0);
		...
		let priceSushi = SushiPairToken0 === Usdt.address ? sushiReserves[0].div(sushiReserves[1]) : sushiReserves[1].div(sushiReserves[0]);

		// Check pango pair price
		let pangoReserves = await PangoWavaxUsdt.getReserves();
		let pricePango = SushiPairToken0 === Usdt.address ? pangoReserves[0].div(pangoReserves[1]) : pangoReserves[1].div(pangoReserves[0]);
	...
	it('should send user .5 WAVAX', async function () {
		...
		const SushiPairToken0 = await SushiSwapWavaxUsdt.token0();
		// If token0 is USDT then set amount0 to  0 else set its value to 1 since its WAVAX
		const amount0 = SushiPairToken0 === Usdt.address ? arbitrageAmount : expandTo18Decimals(0);

		// If token0 is USDT then set amount1 to 1 else set its value to 0 since its USDT
		const amount1 = SushiPairToken0 === Usdt.address ? expandTo18Decimals(0) : arbitrageAmount;
