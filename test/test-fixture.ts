...
const setupTest = deployments.createFixture(async ({ deployments, getNamedAccounts, ethers }, options): Promise<V2Fixture> => {
   ...
   const pangolinFactory = await deploy('PangolinFactory', {
    ...
    const UNISWAP_V2_FACTORY = new Contract(uniswapFactory.address, uniswapFactory.abi, signers[0]);
    ...
    const SUSHI_SWAP_ROUTER = new Contract(sushiSwapRouter.address, sushiSwapRouter.abi, signers[0]);
    ...
    const PANGOLIN_ROUTER = new Contract(pangolinRouter.address, pangolinRouter.abi, signers[0])

    await PANGO_FACTORY.createPair(WAVAX.address, USDT.address);
    const PANGO_WAVAX_USDT_PAIR_ADDRESS = await PANGO_FACTORY.getPair(WAVAX.address, USDT.address);
    const PangoPair = await deployments.getArtifact('IPangolinPair');
    const PANGO_WAVAX_USDT_PAIR = new Contract(PANGO_WAVAX_USDT_PAIR_ADDRESS, PangoPair.abi, signers[1]);

    await UNISWAP_V2_FACTORY.createPair(WAVAX.address, USDT.address);
    const UNISWAP_WAVAX_USDT_PAIR_ADDRESS = await UNISWAP_V2_FACTORY.getPair(WAVAX.address, USDT.address);
   ...
   const flashSwapSushiPango = await deploy("FlashSwapSushiPango", {
    ...
    const FLASH_SWAP_SUSHI = new Contract(flashSwapSushiPango.address, flashSwapSushiPango.abi, signers[0]);
